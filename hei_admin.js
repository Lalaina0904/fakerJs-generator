...
for (let i = 0; i < 1000; i++) {
    const id_group =
        faker.datatype.number() + faker.random.alpha({ casing: "upper" });
    const ref_group = faker.helpers.unique(
        () =>
            faker.random.alpha({ count: 1, casing: "upper" }) +
            faker.datatype.number({ min: 1, max: 9 }) +
            i
    );
    const creation_datetime = faker.date
        .between("2021-01-01", "2023-06-18")
        .toISOString()
        .slice(0, 19)
        .replace("T", " ");
    ...
    const id_user =
        ...
        faker.random.alpha({ casing: "upper", count: 2 });
    ...
    const ref = faker.helpers.unique(
        () => `STD${faker.datatype.number(9999).toString()}` + i
    );
    let birth_date = faker.date
        .between("1980-01-01", new Date())
        .toISOString()
        .slice(0, 10);
    const entrance_datetime = faker.date
        .between("2021-01-01", "2023-06-18")
        .toISOString()
        .slice(0, 10);
    ...
    const phone =
        prefixPhone === 0
            ? faker.phone.number("034 ## ### ##")
            : prefixPhone === 1
            ? faker.phone.number("032 ## ### ##")
    ...
    const id_course =
        faker.datatype.number() + faker.random.alpha({ casing: "upper" });
    const ref_course = faker.helpers.arrayElement([
        "PROG1",
        "PROG2",
        "WEB1",
        "WEB2",
    ...
    const name_course = faker.helpers.arrayElement([
        "Algorithme",
        "Javascipt",
        "Bootstrap",
        "Java",
    ...
    const point = faker.datatype.float(
        { min: 0 },
        { max: 20 },
        { precision: 0.5 }
    );
